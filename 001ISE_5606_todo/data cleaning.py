...
# 室厅数转换 平均值取整
def process_room(room_str):
    numbers = [int(s.replace('室', '').strip()) for s in room_str.split(',')]
...
print('对数变换：已增加列数', data.shape[1]-ds2, '清洗后数据规模', data.shape)
ds2 = data.shape[1]

# 检查清洗后数据
data.info()
print('干净数据规模：', data.shape)
print(data.sample(5))

# 输出清洗后数据文件
file_path1 = '../002Data/clean data.csv'
data.to_csv(file_path1, index=False, encoding='utf-8')
...
# 输出拆分标签后数据文件
file_path2 = '../002Data/preprocessing data.csv'
data.to_csv(file_path2, index=False, encoding='utf-8')
